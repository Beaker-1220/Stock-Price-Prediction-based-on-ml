...
def preprocess_daily_data(filepath):
    ...
    df_day = pd.read_csv(filepath, delim_whitespace=True,parse_dates=['Date'], skiprows=1, names=column_names)
...
def preprocess_minute_data(filepath):
    ...
    with open(filepath, 'r') as file:
        column_names = file.readline().strip().split(',')
    df_minute = pd.read_csv(filepath, delim_whitespace=True,parse_dates=['Date'] ,skiprows=1, names=column_names)
    ...
    # 处理日期列可能的NaN值
    df_minute['Date'] = pd.to_datetime(df_minute['Date'], errors='coerce')
...
daily_data.to_csv('processed_daily_data.csv', index=False)

minute_data.to_csv('processed_minute_data.csv', index=False)
